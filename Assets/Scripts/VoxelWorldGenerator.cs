...
public class VoxelWorldGenerator : MonoBehaviour
{
                   ...
                   Vector3Int size, int yMin, int yMax, float density)
    {
        ...
        int oy = rng.Next(Mathf.Max(1, yMin), Mathf.Min(maxY, Mathf.Max(yMin, yMax - size.y + 1)));
    ...
    {
        if (t01 < caveMidHeight01)
            return Mathf.Lerp(caveThresholdBottom, caveThresholdMid, t01 / caveMidHeight01);
        else
            return Mathf.Lerp(caveThresholdMid, caveThresholdTop, (t01 - caveMidHeight01) / (1f - caveMidHeight01));
    }
    ...
    float FBm3D01(int x, int y, int z, float scale, int octs, float pers, float lac, Vector3 offA, Vector3 offB)
    {
        ...
        for (int i = 0; i < octs; i++)
        {
            float nx1 = (x + offA.x) / scale * freq;
            float ny1 = (y + offA.y) / scale * freq;
            float nz1 = (z + offA.z) / scale * freq;
            float nx2 = (x + offB.x) / (scale * 0.85f) * freq;
            float ny2 = (y + offB.y) / (scale * 0.85f) * freq;
            float nz2 = (z + offB.z) / (scale * 0.85f) * freq;
